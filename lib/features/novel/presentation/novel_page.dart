...
class NovelPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(t.novels)),
      body: entries.when(
        data: (items) {
          if (items.isEmpty) return Center(child: Text(t.noNovelUpdates));
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                child: ListTile(
                  ...
                  subtitle: Text(t.latestChapter(_chapterName(latest),
                      formatShortDate(latest.publishedAt ?? latest.fetchedAt))),
                  trailing: latest.isRead
                      ? const Icon(Icons.done)
                      : const Icon(Icons.play_arrow),
                  onTap: () => Navigator.of(context).push(MaterialPageRoute(
                      builder: (_) => ReaderPage(entryId: latest.id))),
                ),
              );
            },
          );
        ...
        error: (error, _) => Center(child: Text(t.failedToLoadNovels(error))),
      ),
    );
  }
  ...
  String _novelTitle(Entry entry) {
    ...
    return (extra['novelTitle'] as String?)?.trim().isNotEmpty == true
        ? extra['novelTitle'] as String
        : entry.sourceName;
  }

  String _chapterName(Entry entry) {
    final extra = _extra(entry);
    return (extra['chapterName'] as String?)?.trim().isNotEmpty == true
        ? extra['chapterName'] as String
        : entry.title;
  }
